...
file.lookup <- function(machine,date){
  ...
  ## date is a date formatted yymmdd (eg. 150310)
  
  data.files <- list.files(machine,full.names=T)
  ## returns list of files for the specified machine
  
  ids.files <- list.files(paste0(machine,"/runfiles"),full.names=T)
  ## returns list of files in the 'runfiles' folder for the 
  ## specified machine, ie. the sample ids
  
  data.file <- tail(grep(date, data.files, value=TRUE), 1)
  ## returns filepath for last isotope file generated
  ## for the specified machine on the specified date

  ids.file <- tail(grep(date, ids.files, value=TRUE), 1)
...
file is correctly formatted")} else {warning("data file correct")}
  ...
  ids <- read.csv(filenames$ids.file,stringsAsFactors=FALSE,
                  strip.white=TRUE)
...
process.data <- function(filenames){
  ## filenames is a list with filenamea for a data file & a sample
  ## ids file, such as that generated by the file.lookup function
  
  df <- data.mod(data.file = filenames$data.file, 
                 ids.file = filenames$ids.file)
  ...
  qa.report <- qa.summary(filenames$data.file,qa.file,mem,drift,
                          cal,flagged)
...
#####################################################################
review.data <- function(data){
  ## data is a list of dataframes such as that created by the 
  ## process.crds function that includes qa.report & samples.summary
  
  print.me <- print.format(data)
  ## runs print.format function

  plot.gmwl(print.me$samples.summary)
